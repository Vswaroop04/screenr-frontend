...
export const screeningKeys = {
  ...
  verixConversations: (jobId: string) =>
    ['verix', jobId, 'conversations'] as const,
...
export function useRecomputeRanking (jobId: string) {
  ...
  return useMutation({
    mutationFn: (resumeIds?: string[]) =>
      recruiterAPI.recomputeRanking(jobId, resumeIds),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: screeningKeys.jobCandidates(jobId)
      })
...
export function useCandidateResumeDownload () {
  return useMutation({
    mutationFn: async (resumeId: string) => {
      const { downloadUrl } = await candidateAPI.getMyResumeDownloadUrl(
        resumeId
      )
      window.open(downloadUrl, '_blank')
      return downloadUrl
